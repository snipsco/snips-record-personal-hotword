...
**Note 1**: if you encounter errors becouse your microphone is not availale, this might be because it is currently used by `snips-audio-server`. If yes, stop it bbefore lauching the previous script by running `sudo systemctl stop snips-audio-server`.
